...
In this approach, government — in partnership with an outside vendor — works with end-users of the planned system to determine what their needs are and how to meet those needs through software and process/service improvements. The Government establishes a set of target goals or outcomes and some measurable indicators to accompany them. Government and the vendor then break down these higher level goals into individual hypotheses, that can be tested and validated (or refuted) with prototypes and usability testing. Once there is some initial validation of how to get closer to the target outcomes, the Government and vendor create a backlog of features to be added to the software, and the vendor uses that as a long, detailed to-do list, which is constantly modified in response to evolving needs. Every couple of weeks, the completed work is handed over to government for government inspection, deployment, and testing with real users. There, its audience can start to use it, and government and the vendor can see if they have successfully addressed the users' needs. It's this tight feedback loop — research, develop, deliver, repeat — that government must own, because it is the best way to ensure that users get the software tools they need and that government gets what it's paying for.
